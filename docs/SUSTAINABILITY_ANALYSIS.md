...
Legacy actual-versus-canonical, CodeCarbon-energy, and CO2 summaries are not part of the supported
v1.1 evidence. In particular, the removed files `actual_vs_canonical.json`,
...
The last of those rendered the untrusted CodeCarbon `energy_kwh` and `co2_kg` fields as per-phase
Wh and gram-CO2 panels and therefore contradicted the boundary stated above. The underlying
