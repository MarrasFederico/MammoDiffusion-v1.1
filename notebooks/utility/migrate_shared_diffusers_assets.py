...
from shared_diffusers_assets import (
    DIFFUSERS_REVISION, find_legacy_diffusers_copies,
    find_legacy_sd21_base_copies, resolve_shared_diffusers_repo,
...
def audit(root: Path | None = None) -> dict:
    ...
    repos, models = [], []
    for path in find_legacy_diffusers_copies(root):
    ...
    for path in find_legacy_sd21_base_copies(root):
        try:
    ...
    return {"shared_diffusers": {"path": str(shared_repo), **repo_info},
            ...
            "legacy_diffusers": repos, "legacy_sd21": models,
...
def main() -> None:
    ...
    parser.add_argument("--remove-verified-old-diffusers-repos", action="store_true")
    parser.add_argument("--remove-verified-old-sd21-copies", action="store_true")
    args = parser.parse_args(); report = audit()
    print(report)
    for item in report["legacy_diffusers"]:
        path = Path(item["path"])
        if args.remove_verified_old_diffusers_repos and item.get("revision") == DIFFUSERS_REVISION and not item.get("dirty") and not has_active_symlink_dependency(shared_diffusers_assets.PROJECT_ROOT, path):
            shutil.rmtree(path)
    for item in report["legacy_sd21"]:
        path = Path(item["path"])
        if args.remove_verified_old_sd21_copies and item.get("matches_shared") and not has_active_symlink_dependency(shared_diffusers_assets.PROJECT_ROOT, path):
            shutil.rmtree(path)
