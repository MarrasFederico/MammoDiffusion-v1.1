...
def _resolve_project_path(project_root: Path, value: object) -> Path:
    """Resolve a manifest path through the shared project-path rules.

    See ``project_paths``: a historical absolute prefix is reduced to its
    repository-relative suffix and re-anchored here, never followed verbatim.
    An absolute path with no recognisable marker is returned unchanged so the
    caller's "resolves outside the project root" diagnostic still reports it.
    """
    return project_paths.resolve_project_path(project_root, value, must_stay_inside=False)
