...
# ── Carica VAE best (già addestrato) ─────────────────────────────────────────
...
def cosine_schedule(num_steps, s=0.008):
    ...
    return tf.clip_by_value(betas, 1e-4, 0.9999)
...
print(f"LDM U-Net params: {ldm_model.count_params():,}")
...
def ldm_train_step(z0, y):
    ...
    with tf.GradientTape() as tape:
        ...
        eps_pred, v_pred = tf.split(out, 2, axis=-1)
        loss_simple      = tf.reduce_mean(tf.square(noise - eps_pred))
        loss_vlb         = vb_term_ldm(z0, z_t, t, eps_pred, v_pred)
