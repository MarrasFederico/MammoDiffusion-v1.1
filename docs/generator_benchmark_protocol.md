...
## Representation-aware technical validity

Feature extractability and image quality are reported separately. An image is feature-extractable when it is readable, has the expected shape, and produces a non-empty finite numeric array. A quality-valid image is additionally neither near-black nor constant-range. The compatibility field `n_technically_valid` is an alias of `n_quality_valid`; it is not the feature-execution gate.

RAW quality defects are reported and retained in RAW metrics. Near-black and constant-range RAW images remain in the canonical feature-extractable pool as warnings, so RAW metrics reflect authentic generator failures and the effect of filtering. Corrupt, unsupported-shape, non-numeric, or insufficiently numerous/unique feature-extractable RAW pools are fatal only for that RAW representation.

FILTERED is the representation used for official family ranking, and FILTERED eligibility remains strict. Corrupt, wrong-shape, near-black, and constant-range files are fatal; at least 1,361 quality-valid unique images and the protocol duplicate-rate gate are required. A RAW warning never invalidates a clean canonical FILTERED pool. The benchmark stops before feature loading only when no official FILTERED candidate remains in an entire scientific family.

If an extractor produces a non-finite feature, the affected representation is blocked with its sample ID, path, extractor, and cause. The image is never silently removed, and the other representation is evaluated independently.
...
Technical validity and filtering acceptance are independent. Acceptance rate is a descriptive selection-pressure/efficiency measure, not a universal exclusion gate and not a direct comparison across deliberately different RAW pool sizes. Eligibility instead requires a valid canonical filter manifest, complete RAW→FILTERED mapping, at least 1,361 quality-valid unique FILTERED images, no corrupt files, complete provenance and a canonical CSV training-corpus manifest. A protocol-specific top-K run passes when its declared target is reached even when acceptance is about 1,361/4,083.

Canonical publication-v2 identity is `(sample_id, project-relative path, SHA-256)`. Basenames are never sufficient. The canonical flat `generator_summary.csv` reports `raw_feature_extractable_rate`, `raw_quality_validity_rate`, `raw_near_black_rate`, `raw_constant_range_rate`, `filtered_feature_extractable_rate`, and `filtered_quality_validity_rate` descriptively. These RAW fields are not ranking tie-breaks. Ranking fields remain, in order, `raddino_kid` (RAD-DINO KID full-pool), `raddino_coverage` (balanced point), `raddino_precision` (balanced point), descriptive `raddino_fid` (full-pool), `inception_kid` (full-pool), `raddino_kid_std` (stability standard deviation), and `generator_id`.
