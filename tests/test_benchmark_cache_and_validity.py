...
class RuntimeCandidateAuditTests(unittest.TestCase):
    ...
    def _fixture(self, root: Path):
        ...
        return entry, raw_paths, filtered_paths

    def test_runtime_audit_needs_only_concrete_assets_not_retired_records(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary); entry, _, _ = self._fixture(root)
            audit = gb.audit_candidate(root, entry, self.protocol)
            self.assertTrue(audit["eligible_for_benchmark_execution"])
